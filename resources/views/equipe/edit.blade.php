...
<div class="title-wrapper pt-30">
    <div class="row align-items-center">
        <div class="col-md-12">
            <div class="title">
                <h2>Modifier l'équipe pour {{$chantier->client->nom_client}}  : {{ $chantier->client->code_client }} - {{ $chantier->sousTypeMission->types ?? '-' }}</h2>
            </div>
        </div>
    </div>
</div>

<form action="{{ route('equipe.update', $chantier->id_chantier) }}" method="POST">
    @csrf
    @method('PUT')
                    ...
                    <div class="dropdown-content">
                        ...
                        <label>
                            <input type="checkbox" name="personnel[{{ $grade->id_grade }}][]" value="{{ $personnel->id_liste_personnel }}"
                                @if(isset($equipe[$grade->id_grade]) && in_array($personnel->id_liste_personnel, $equipe[$grade->id_grade]->pluck('id_liste_personnel')->toArray()))
                                    checked
                                @endif>
                            {{ $personnel->prenom }}
                        </label>
...
.dropdown-content {
    ...
    min-width: 300px; /* Largeur de la liste déroulante */
