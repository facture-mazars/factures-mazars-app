...
<!-- end col -->
                    ...
                    @foreach ($encaissement as $c)
                      <tr>
                        ...
                        @if(($c->reste_a_payer ?? 0) != ($c->montant_a_encaisse ?? 0))
                          <a href="{{ route('encaissement.create', ['id_tranche_facture' => $c->trancheFacture->id_tranche_facture]) }}" class="main-btn info-btn-light rounded-full btn-hover"">Encaisser</a>
                        @endif
                        </td>
    ...
    @if($c->trancheFacture->date_facture_annule)
        <a href="{{ route('tranche.detailsansEncaissAnnuler', ['id_tranche_facture' => $c->id_tranche_facture]) }}" class="text-gray">
            Voir Facture 
        </a>
    @else
        <a href="{{ route('tranche.detailsansEncaiss', ['id_tranche_facture' => $c->id_tranche_facture]) }}" class="text-gray">
            Voir facture
        </a>
    @endif
</li>
