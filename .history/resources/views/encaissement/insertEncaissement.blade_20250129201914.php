        ...
        <div class="select-position">
            <select class="light-bg" id="id_banque" name="id_banque">
                ...
                @foreach($banques as $c)
                    <option value="{{ $c->id_banque }}">{{ $c->type_banque }}-{{ $c->type_monnaie }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<div class="col-xxl-4">
    <div class="input-style-1">
        ...
        <input style="color: red; font-family : bold; font-size : 30px;" 
               type="text" id="reste_a_payer" 
               name="reste_a_payer" 
               value="{{ $total_a_payer }}" 
               readonly>
    </div>
...
// Fonction pour mettre à jour le montant restant à payer lors de l'insertion du montant à encaisser
document.getElementById('montant_a_encaisse').addEventListener('input', function(event) {
    ...
    document.getElementById('reste_a_payer').value = formatNumber(resteAPayer.toString());
});
