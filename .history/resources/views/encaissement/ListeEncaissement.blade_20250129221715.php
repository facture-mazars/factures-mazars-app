...
<td>
                          <span class="text-sm">{{ $c->trancheFacture->facture->chantier->monnaie->nom_monnaie }}</span>      
</td>

@if (Auth::user()->role === 'Admin')    
                        <td>
                       {{}} @if(($c->reste_a_payer ?? 0) != ($c->montant_a_encaisse ?? 0) && ($c->reste_a_payer ?? 0) > 0)
                          <a href="{{ route('encaissement.create', ['id_tranche_facture' => $c->trancheFacture->id_tranche_facture]) }}" class="status-btn info-btn-light rounded-full btn-hover"">Payer</a>
                        @endif
                        </td>
    @endif                    
                          ...
                          @if($c->trancheFacture->date_facture_annule)
                              <a href="{{ route('tranche.detailsansEncaissAnnuler', ['id_tranche_facture' => $c->id_tranche_facture]) }}" class="status-btn secondary-btn-light rounded-full btn-hover">
                              <i class="lni lni-eye"></i> Facture 
                              </a>
                          @else
                              <a href="{{ route('tranche.detailsansEncaiss', ['id_tranche_facture' => $c->id_tranche_facture]) }}" class="status-btn secondary-btn-light rounded-full btn-hover">
                              <i class="lni lni-eye"></i>  facture
                              </a>
                          @endif
