...
class ChoixBanqueController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $choix_banque = ChoixBanque::where('id_banque', $banque->id_banque)->first();

        if (!$choix_banque) {
            // Si aucun enregistrement n'est trouvé, rediriger vers la page de choix
            return redirect()->route('choix.create', ['id_facture' => $request->input('id_facture')])
                             ->with('success', 'Banque mise à jour, veuillez choisir un choix de banque.');
        }
    
        // Si l'enregistrement existe, récupérer id_facture
        $id_facture = $choix_banque->id_facture;


        dd($id_facture);  // Ajoutez ceci avant la redirection pour voir si l'id_facture est bien passé

        // Redirection avec id_facture passé en paramètre
        return redirect()->route('choix.create', ['id_facture' => $id_facture])
