...
class ChoixBanqueController extends Controller
{
     ...
     */
     ...
     public function edit($id)
     {
         ...
         return response()->json($banque);  // Retourner les données de la banque en JSON
     }
 
     // Méthode pour mettre à jour la banque
     public function update(Request $request, $id)
     {
         $banque = Banque::findOrFail($id);  // Trouver la banque par ID
         $banque->update([
             'nom_banque' => $request->nom_banque,
             'compte' => $request->compte,
             'type' => $request->type
         ]);
         return redirect()->route('banques.index')->with('success', 'Banque mise à jour avec succès');
     }
