...
class EncaissementController extends Controller
{
    ...
    public function create($id_tranche_facture)
    {
        ...
        $total_encaisse = Encaissement::where('id_tranche_facture', $id_tranche_facture)
        ->sum('montant_a_encaisse');

// Calculer le reste à payer
$reste_a_payer = $total_a_payer - $total_encaisse;

        return view('encaissement.insertEncaissement', compact('tranche_facture', 'banques','cheques','total_a_payer','total_encaisse','reste_a_payer'));
    }
    ...
    if ($request->has('reste_a_payer')) {
        ...
        $request->validate([
          ...
          // Récupérer la tranche de facture et le montant total à payer
    $trancheFacture = TrancheFacture::findOrFail($request->input('id_tranche_facture'));
   
    // Vérifier si la tranche est déjà encaissée
    ...
    $encaissementMontant = $request->input('montant_a_encaisse');
    $factureAEncaisser = FactureAEncaisser::where('id_tranche_facture', $trancheFacture->id_tranche_facture)->first();
    $totalAPayer = $factureAEncaisser->total_a_payer;

    // Mettre à jour le reste à payer
    $resteAPayer = $totalAPayer - $encaissementMontant;
    
    // Si le montant encaissé atteint le total, marquer la facture comme payée
    if ($resteAPayer <= 0) {
        $this->updateFactureStatus($facture->id_facture); // Mettez à jour l'état de la facture si tout est payé
    }

    // Mettre à jour le champ reste_a_payer dans la table facture_a_encaisser
    $factureAEncaisser->reste_a_payer = $resteAPayer;
    $factureAEncaisser->save();
...
public function updateFactureStatus(Facture $facture): void
{
    // Vérifiez si la facture est complètement payée
    if ($facture->reste_a_payer <= 0) {
        // Marquez la facture comme payée
        $facture->etat_encaissement = 'payée';  // Ou '1' si vous utilisez des entiers pour l'état
        $facture->save();  // Sauvegardez les modifications
    }
}
