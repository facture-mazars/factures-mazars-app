...
$reste_a_payer = $total_a_payer - $total_encaisse;
    ...
    // Calculer le reste à payer
    $total_encaisse = Encaissement::where('id_tranche_facture', $request->id_tranche_facture)
                                  ->sum('montant_a_encaisse');
    $reste_a_payer = $total_a_payer - $total_encaisse;

    // Vérifier si le paiement total est atteint
    if ($reste_a_payer <= 0) {
        return redirect()->back()->withErrors(['error' => 'Le paiement total a déjà été effectué.']);
    }

    // Si ce n'est pas le cas, enregistrer l'encaissement
