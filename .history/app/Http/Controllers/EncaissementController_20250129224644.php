...
class EncaissementController extends Controller
{
    ...
    if ($request->has('reste_a_payer')) {
        ...
        // Récupérer la tranche de facture et la facture associée
    $trancheFacture = TrancheFacture::findOrFail($request->input('id_tranche_facture'));
    $facture = $trancheFacture->facture;

    // Vérification si la tranche est déjà encaissée
    $total_a_payer = FactureAEncaisser::where('id_tranche_facture', $request->id_tranche_facture)->first()->total_a_payer;
    $total_encaisse = Encaissement::where('id_tranche_facture', $request->id_tranche_facture)->sum('montant_a_encaisse');
    
    // Vérifier que le montant à encaisser ne dépasse pas le montant restant
    $reste_a_payer = $total_a_payer - $total_encaisse;
    if ($request->montant_a_encaisse > $reste_a_payer) {
        return redirect()->back()->withErrors(['montant_a_encaisse' => 'Le montant à encaisser ne peut pas être supérieur au reste à payer.']);
    }

    // Récupérer le dernier ID d'encaissement et créer un nouveau ID
    ...
    $encaissement = new Encaissement([
        ...
        'reste_a_payer' => $reste_a_payer - $request->montant_a_encaisse, // Met à jour le reste à payer après encaissement
        'created_at' => now(),
        'updated_at' => now(),
    ]);
    ...
    // Mettre à jour la tranche de facture si tout est payé
    if ($total_encaisse + $request->montant_a_encaisse >= $total_a_payer) {
        $trancheFacture->etat = true; // Tranche entièrement réglée
        $trancheFacture->save();
    }
