...
class EncaissementController extends Controller
{
    ...
    // Vérifier si la facture a déjà été partiellement encaissée
    $encaissementExistant = Encaissement::where('id_tranche_facture', $request->id_tranche_facture)
                                       ->whereNull('montant_a_encaisse')  // Si un encaissement partiel existe
                                       ->first();

    if ($encaissementExistant) {
        // Mettre à jour l'encaissement existant
        $encaissementExistant->montant_a_encaisse += $request->montant_a_encaisse;  // Ajouter le montant à encaisser
        $encaissementExistant->reste_a_payer = $request->reste_a_payer;  // Mettre à jour le reste à payer
        $encaissementExistant->save();
    } else {
        // Créer un nouvel encaissement si aucun encaissement partiel n'existe
        $lastEncaissement = Encaissement::orderBy('id_encaissement', 'desc')->first();
        $newId = $lastEncaissement ? $lastEncaissement->id_encaissement + 1 : 1;
