...
$reste_a_payer = $total_a_payer - $total_encaisse;
    ...
    if ($request->has('reste_a_payer')) {
        ...
        // Vérifier si l'encaissement existe déjà pour cette tranche
        $encaissement = Encaissement::where('id_tranche_facture', $request->id_tranche_facture)->first();
    
        // Si aucun encaissement n'existe, créer un nouvel enregistrement
        if (!$encaissement) {
            ...
            $encaissement->datereel_encaissement = $request->datereel_encaissement;
            $encaissement->id_banque = $request->id_banque;
            $encaissement->id_cheque = $request->id_cheque;
            $encaissement->montant_a_encaisse = $request->montant_a_encaisse;
            $encaissement->reste_a_payer = $request->reste_a_payer;
            $encaissement->updated_at = now();
            $encaissement->save();
        }
