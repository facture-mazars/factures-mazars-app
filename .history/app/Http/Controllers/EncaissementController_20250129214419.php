...
class EncaissementController extends Controller
{
    ...
    {
        // Supprimer les séparateurs de milliers et autres caractères non numériques
        ...
        $facture->updateFactureStatus($facture->id_facture);
        ...
        // Mettre à jour le reste à payer de la tranche de facture
        $totalEncaisse = Encaissement::where('id_tranche_facture', $request->id_tranche_facture)->sum('montant_a_encaisse');
        $totalFacture = $facture->montant_total; // suppose que le montant total de la facture est dans la colonne `montant_total` de la table `facture`
    
        $reste_a_payer = $totalFacture - $totalEncaisse;
    
        // Mettre à jour le champ `reste_a_payer` dans la tranche de facture
        $trancheFacture->reste_a_payer = $reste_a_payer;
        $trancheFacture->save();
